...
//    ];
    ...
    public function run(): void
    {
        ...
        $response = file_get_contents("https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc"
            , false,
            $context
        );
        ...
        foreach ($data['results'] as $movie) {
            $videoResponse = file_get_contents("https://api.themoviedb.org/3/movie/" . $movie['id'] . "/videos?language=en-US"
                , false,
                $context
            );
            ...
            $dummy_data[] = [
                ...
                "video_url" => "https://www.youtube.com/embed/" . $videoData['results'][0]['key']
            ];
        }

        foreach ($dummy_data as $movie) {
